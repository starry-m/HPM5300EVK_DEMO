...
void init_spi_pins_with_gpio_as_cs(SPI_Type *ptr)
{
    if (ptr == HPM_SPI1) {
        ...
        HPM_IOC->PAD[IOC_PAD_PB05].FUNC_CTL = IOC_PB05_FUNC_CTL_SPI3_SCLK | IOC_PAD_FUNC_CTL_LOOP_BACK_MASK;
        HPM_IOC->PAD[IOC_PAD_PB07].FUNC_CTL = IOC_PB07_FUNC_CTL_SPI3_MOSI;
        HPM_IOC->PAD[IOC_PAD_PB06].FUNC_CTL = IOC_PB06_FUNC_CTL_SPI3_MISO;
 ...
 void init_pins(void)
{
    ...
    HPM_IOC->PAD[IOC_PAD_PB04].FUNC_CTL = IOC_PB04_FUNC_CTL_SPI3_CS_0;
    ...
    gpiom_disable_pin_visibility(HPM_GPIOM, GPIOM_ASSIGN_GPIOB, 10, gpiom_core0_fast);
    gpio_set_pin_output(HPM_GPIO0, GPIO_OE_GPIOB, 10);
    gpio_write_pin(HPM_GPIO0, GPIO_DO_GPIOB, 10, 0);
}
  */
